---
title: Giving AgentGateway a Semantic Brain with vLLM Semantic Router - Inside My Homelab
description: Integrating AgentGateway with vLLM semantic router for efficient multiplexing.
date: '2026-06-22'
draft: false
slug: '/pensieve/agentgateway-semantic'
tags:
  - vLLM
  - MCP
  - Rust
---

## Connecting the Pieces
AgentGateway serves as an incredible multiplexer for MCP, but combining it with a proper semantic routing layer using `vllm-sr` unlocks entirely new capabilities...

*(Content placeholder for full blog post)*
